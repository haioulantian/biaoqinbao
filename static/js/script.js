document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSelectionDecorations() {
    // Phase 1: read — collect all DOM info without writing
    const emojiUpdates = [];
    document.querySelectorAll(".emoji-item").forEach((emojiItem) => {
      const category = emojiItem.dataset.category;
      const emoji = emojiItem.dataset.emoji;
      const selected = isEmojiSelected(category, emoji);
      const indicator = emojiItem.querySelector(".selection-indicator");
      const needsModeClass = selectionState.enabled !== emojiItem.classList.contains("selection-mode");
      const needsSelectedClass = selected !== emojiItem.classList.contains("selected");
      const needsCheckedClass = indicator && selected !== indicator.classList.contains("checked");
      emojiUpdates.push({ emojiItem, indicator, selected, needsModeClass, needsSelectedClass, needsCheckedClass });
    });
    ...
    document.querySelectorAll(".category").forEach((categoryDiv) => {
      ...
      categoryUpdates.push({ categoryDiv, summary, selectAllBtn, totalCount, selectedCount, hasEmojis, allSelected });
    });

    // Phase 2: write — apply all changes in one frame
    requestAnimationFrame(() => {
      for (const u of emojiUpdates) {
        if (u.needsModeClass) {
          u.emojiItem.classList.toggle("selection-mode", selectionState.enabled);
        }
        if (u.needsSelectedClass) {
          u.emojiItem.classList.toggle("selected", u.selected);
        }
        if (u.indicator && u.needsCheckedClass) {
          u.indicator.classList.toggle("checked", u.selected);
          u.indicator.setAttribute("aria-label", u.selected ? "已选中" : "未选择");
        }
      }

      for (const u of categoryUpdates) {
        if (u.summary) {
          u.summary.textContent = selectionState.enabled
            ? `已选 ${u.selectedCount} / ${u.totalCount}`
            : "未开启批量选择";
        }
        if (u.selectAllBtn) {
          u.selectAllBtn.disabled = !u.hasEmojis;
          u.selectAllBtn.textContent = selectionState.enabled
            ? u.allSelected
              ? "取消本类"
              : "本类全选"
            : "本类选择";
  ...
  function resetDangerConfirmState() {
    console.log("[DEBUG] resetDangerConfirmState called");
  ...
  function startDangerCountdown() {
    console.log("[DEBUG] startDangerCountdown called, stage:", dangerConfirmStage, "config:", dangerConfirmConfig);
    if (dangerConfirmStage !== "ack" || !dangerConfirmConfig) {
      console.log("[DEBUG] startDangerCountdown early return - stage:", dangerConfirmStage, "hasConfig:", !!dangerConfirmConfig);
      return;
    }

    const countdown = dangerConfirmConfig?.countdown ?? 0;
    ...
    if (countdown <= 0) {
      // 无倒计时，直接可执行
      console.log("[DEBUG] countdown <= 0, going to ready state");
      dangerConfirmStage = "ready";
      if (dangerModalStageText) {
        dangerModalStageText.textContent = "已确认，请点击下方按钮执行。";
      }
      if (dangerModalConfirmBtn) {
        dangerModalConfirmBtn.disabled = false;
        dangerModalConfirmBtn.textContent = dangerConfirmConfig.actionLabel;
        console.log("[DEBUG] confirm button enabled, text:", dangerConfirmConfig.actionLabel);
      }
      return;
    }
    ...
    dangerConfirmTimer = setInterval(() => {
      ...
      dangerConfirmTimer = null;
      dangerConfirmStage = "ready";
      dangerModalStageText.textContent = "倒计时已结束，请点击下方按钮执行。";
      dangerModalConfirmBtn.disabled = false;
      dangerModalConfirmBtn.textContent = dangerConfirmConfig.actionLabel;
    }, 1000);
  }

  function showDangerConfirm({ title, description, actionLabel, countdown = 0 }) {
    console.log("[DEBUG] showDangerConfirm called:", { title, description, actionLabel, countdown });
    if (
      !dangerModalRoot ||
      !dangerModalTitle ||
      ...
      console.log("[DEBUG] Falling back to native confirm - missing elements:", {
        dangerModalRoot: !!dangerModalRoot,
        dangerModalTitle: !!dangerModalTitle,
        dangerModalDescription: !!dangerModalDescription,
        dangerModalConfirmBtn: !!dangerModalConfirmBtn,
      });
    ...
    if (dangerModalStageText) {
      dangerModalStageText.textContent = countdown > 0
        ? `请先勾选已理解，勾选后会自动开始 ${countdown} 秒倒计时。倒计时结束后才可执行。`
        : "请先勾选已理解，勾选后即可执行。";
    }
    if (dangerModalConfirmBtn) {
      dangerModalConfirmBtn.textContent = "请先勾选上方选项";
    ...
    console.log("[DEBUG] Removing hidden class from danger modal");
  ...
  function buildLightboxImageList(currentCategory, currentEmoji) {
    lightboxImages = [];
    let foundIndex = -1;
    for (const [category, emojis] of Object.entries(latestEmojiData)) {
      if (!Array.isArray(emojis)) continue;
      emojis.forEach((emoji) => {
        if (category === currentCategory && emoji === currentEmoji) {
          foundIndex = lightboxImages.length;
        }
        lightboxImages.push({ category, emoji });
      });
    }
    return foundIndex;
  }

  function openLightbox(category, emoji) {
    const lightbox = document.getElementById("lightbox");
    const lightboxImg = document.getElementById("lightbox-img");
    if (!lightbox || !lightboxImg) return;

    const idx = buildLightboxImageList(category, emoji);
    if (idx < 0) return;
    lightboxIndex = idx;
  ...
  function showLightboxImage() {
    ...
    if (!lightboxImg || lightboxIndex < 0 || lightboxIndex >= lightboxImages.length) return;

    const { category, emoji } = lightboxImages[lightboxIndex];
    lightboxImg.src = `/memes/${encodeURIComponent(category)}/${encodeURIComponent(emoji)}`;
    lightboxImg.alt = emoji;

    if (lightboxInfo) {
      lightboxInfo.textContent = `${emoji} — ${category} (${lightboxIndex + 1} / ${lightboxImages.length})`;
    }
  }
  ...
  async function clearAllEmojiFiles() {
    console.log("[DEBUG] clearAllEmojiFiles called, latestEmojiData keys:", Object.keys(latestEmojiData).length);
    const categoryCount = Object.keys(latestEmojiData).length;
    const totalEmojiCount = Object.values(latestEmojiData).reduce(
      (sum, emojis) => sum + (Array.isArray(emojis) ? emojis.length : 0),
      0
    );
    if (categoryCount === 0) {
      showToast("当前没有可清空的内容", "warning", "无需清空");
      return;
    }

    const confirmed = await showDangerConfirm({
      title: "清空全部表情包和分类",
      description: `该操作会删除全部 ${categoryCount} 个分类及其 ${totalEmojiCount} 个表情包，同时清空配置。此操作不可恢复。`,
      actionLabel: "确认清空全部",
    });
    if (!confirmed) {
      return;
    ...
    try {
      ...
      await refreshUi({ emojis: true });
      showToast(
        `已清空全部内容，删除了 ${data.deleted_categories.length} 个分类，共 ${data.deleted_files_total} 个文件。`,
        "success",
        "清空成功",
        4200
      );
  ...
  if (clearAllBtn) {
    clearAllBtn.addEventListener("click", () => {
      console.log("[DEBUG] clear-all-btn clicked");
      clearAllEmojiFiles().catch(err => {
        console.error("[DEBUG] clearAllEmojiFiles error:", err);
        showToast("操作失败：" + err.message, "error", "错误");
      });
    });
  }
  ...
  if (dangerModalAcknowledge) {
    dangerModalAcknowledge.addEventListener("change", () => {
      console.log("[DEBUG] danger checkbox changed, checked:", dangerModalAcknowledge.checked, "stage:", dangerConfirmStage);
      if (dangerConfirmStage === "ack") {
        if (!dangerModalAcknowledge.checked) {
          console.log("[DEBUG] checkbox unchecked, disabling confirm");
          dangerModalConfirmBtn.disabled = true;
          dangerModalConfirmBtn.textContent = "请先勾选上方选项";
          return;
        }
        console.log("[DEBUG] checkbox checked, starting danger countdown");
  ...
  if (dangerModalConfirmBtn) {
    dangerModalConfirmBtn.addEventListener("click", () => {
      console.log("[DEBUG] danger confirm btn clicked, stage:", dangerConfirmStage, "disabled:", dangerModalConfirmBtn.disabled);
      if (dangerConfirmStage === "ack" && dangerModalAcknowledge?.checked) {
        console.log("[DEBUG] Starting countdown from confirm button click");
        startDangerCountdown();
        return;
      }
      if (dangerConfirmStage === "ready") {
        console.log("[DEBUG] Closing danger confirm with true");
  ...
  if (uploadFolderBtn && uploadFolderInput) {
    // 每次点击前清空 value，确保重复选同一文件夹也能触发 change 事件
    uploadFolderInput.addEventListener("click", () => {
    ...
    uploadFolderInput.addEventListener("change", async () => {
      ...
      // 从第一个文件的相对路径中提取文件夹名
      const relativePath = files[0].webkitRelativePath || files[0].relativePath || "";
      let folderName = relativePath.split("/")[0] || "";

      // 如果浏览器不支持 webkitdirectory，回退为手动输入
      if (!folderName) {
        folderName = prompt("请输入分类名称（所选文件的文件夹名）：", "");
        if (!folderName || !folderName.trim()) {
          showToast("已取消：未输入分类名称。", "warning", "已取消");
          return;
        }
      ...
      if (imageFiles.length === 0) {
        showToast("文件夹中没有图片文件。", "warning", "无图片");
        uploadFolderInput.value = "";
        return;
      }

      const originalHtml = uploadFolderBtn.innerHTML;
      uploadFolderBtn.classList.add("busy");
      uploadFolderBtn.innerHTML = `📂 导入中 0/${imageFiles.length}...`;
      ...
      try {
        ...
        await requestJson(
          "/api/category/restore",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ category: folderName, description: `来自文件夹导入` }),
          },
          { defaultErrorMessage: "创建分类失败" }
        );
        ...
        for (let i = 0; i < imageFiles.length; i++) {
          ...
          try {
            await requestJson("/api/emoji/add", {
              method: "POST",
              body: formData,
            }, { defaultErrorMessage: `上传 ${file.name} 失败` });
            successCount++;
          } catch (err) {
          ...
          if ((i + 1) % 10 === 0) {
            uploadFolderBtn.textContent = `📂 上传中 ${i + 1}/${imageFiles.length}...`;
          }
        }
        ...
        if (failCount === 0) {
          showToast(
            `文件夹「${folderName}」导入完成，共 ${successCount} 张表情包。`,
            "success",
            "导入成功",
            4200
          );
          ...
          showToast(
            `文件夹「${folderName}」导入完成，成功 ${successCount} 张，失败 ${failCount} 张。`,
            "warning",
            "导入完成（部分失败）",
            5000
          );
        ...
        showToast(`导入失败：${error.message}`, "error", "导入失败", 4500);
      } finally {
        uploadFolderBtn.classList.remove("busy");
        uploadFolderBtn.innerHTML = originalHtml || '<i class="fas fa-folder-open"></i>上传文件夹';
      }
    });
  }

  // 批量导入文件夹
  const batchUploadFolderBtn = document.getElementById("batch-upload-folder-btn");
  const batchUploadFolderInput = document.getElementById("batch-upload-folder-input");

  if (batchUploadFolderBtn && batchUploadFolderInput) {
    batchUploadFolderInput.addEventListener("click", () => {
      batchUploadFolderInput.value = "";
    });

    batchUploadFolderInput.addEventListener("change", async () => {
      const files = batchUploadFolderInput.files;
      if (!files || files.length === 0) return;

      // 按子文件夹分组
      // webkitRelativePath 格式: "选中文件夹/子文件夹/文件名"
      //   - 选单文件夹: "happy/img.png" → parts[0]="happy"
      //   - 选父文件夹: "my_memes/happy/img.png" → parts[0]="my_memes", parts[1]="happy"
      const folderGroups = {};
      Array.from(files).forEach((f) => {
        const relativePath = f.webkitRelativePath || f.relativePath || "";
        const parts = relativePath.split("/");
        if (parts.length < 2) return;
        // parts.length >= 3 说明选了父文件夹，子文件夹在 parts[1]
        // parts.length == 2 说明直接选了子文件夹，文件夹名在 parts[0]
        const subFolder = parts.length >= 3 ? parts[1] : parts[0];
        if (!folderGroups[subFolder]) folderGroups[subFolder] = [];
        folderGroups[subFolder].push(f);
      });

      const folderNames = Object.keys(folderGroups);
      if (folderNames.length === 0) {
        showToast("未检测到子文件夹。请选择一个包含子文件夹的父文件夹。", "warning", "无子文件夹");
        batchUploadFolderInput.value = "";
        return;
      }
      ...
      folderNames.forEach((name) => {
        const images = folderGroups[name].filter((f) => {
          const ext = f.name.split(".").pop()?.toLowerCase();
          return ["png", "jpg", "jpeg", "gif", "webp", "bmp", "svg"].includes(ext);
        });
        if (images.length > 0) {
          categoryFiles[name] = images;
          totalImages += images.length;
        }
      });

      const categoryNames = Object.keys(categoryFiles);
      if (categoryNames.length === 0) {
        showToast("子文件夹中没有图片文件。", "warning", "无图片");
        batchUploadFolderInput.value = "";
        return;
      }

      const originalHtml = batchUploadFolderBtn.innerHTML;
      batchUploadFolderBtn.classList.add("busy");
      ...
      try {
        for (const [category, imageFiles] of Object.entries(categoryFiles)) {
          // 更新按钮文字
          batchUploadFolderBtn.innerHTML = `📂 ${doneCategories + 1}/${categoryNames.length} 「${category}」...`;

          // 先确保分类存在
          try {
            await requestJson("/api/category/restore", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ category, description: "来自批量导入" }),
            }, { defaultErrorMessage: `创建分类 ${category} 失败` });
          } catch (err) {
          ...
          totalFail += catFail;
          if (catFail > 0) failedCategories.push(category);
          doneCategories++;
        }
        ...
        if (totalFail === 0) {
          showToast(
            `批量导入完成！${categoryNames.length} 个分类，共 ${totalSuccess} 张表情包。`,
            "success",
            "导入成功",
            4200
          );
          ...
          showToast(
            `批量导入完成，成功 ${totalSuccess} 张，失败 ${totalFail} 张` +
            (failedCategories.length > 0 ? `（失败分类: ${failedCategories.join("、")}）` : ""),
            "warning",
            "导入完成（部分失败）",
            5000
          );
        ...
        showToast(`批量导入失败：${error.message}`, "error", "导入失败", 4500);
      } finally {
        batchUploadFolderBtn.classList.remove("busy");
        batchUploadFolderBtn.innerHTML = originalHtml || '<i class="fas fa-layer-group"></i>批量导入文件夹';
        batchUploadFolderInput.value = "";
      }
    });
  }
