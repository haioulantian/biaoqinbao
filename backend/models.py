...
def clear_all_emojis() -> dict[str, object]:
    ...
    if not memes_root.exists():
        return {"deleted_categories": deleted_categories, "deleted_files_total": 0}

    # 统计并删除所有分类目录
    deleted_files_total = 0
    for category_path in sorted(memes_root.iterdir()):
        if not category_path.is_dir():
            continue
        # 统计文件数
        file_count = len(list(_iter_category_image_paths(category_path)))
        deleted_files_total += file_count
        deleted_categories.append(category_path.name)
        # 删除整个分类目录
        shutil.rmtree(category_path)

    # 清空配置（写入空配置，下次重启会重新生成默认配置）
    try:
        with open(MEMES_DATA_PATH, "w", encoding="utf-8") as f:
    ...
    return {
        ...
        "deleted_files_total": deleted_files_total,
    }
